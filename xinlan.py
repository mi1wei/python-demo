...
# 修改为自己需求的目标邮件
title = 'Verification code' #目标邮件的标题
from1 = 'noreply@nebulai.network' #目标邮件的发件人 from不能用作python变量，所以用了任意如from1
fields = '$BODYTEXT-R|\d{6}$' #从邮件HTML源代码中截取第一个链接 详见官网接口说明自定义邮件导出规则部分

# 定义函数无需修改
def checkMail(email,password):
    timestamp = round(time.time()*1000) # 13位时间戳
    sent = timestamp - 600000 #指定邮件发送时间为当前请求时间减去60秒防止时差
    # 发送GET请求
...
if __name__=="__main__":
  ...
  print(checkMail('patriciapearl2101fia@outlook.com',''))