...
def onefootball_drission(metadata: dict):
    ...
    extension_url = f"https://ofc.onefootball.com/s2"
    ...
    try:
        ...
        if page.ele("text=Verify",timeout=5):
            page.ele("text=Verify").wait(1).click('js')
            print(f"浏览器ID: {seq}, Daily check-in Verify")
