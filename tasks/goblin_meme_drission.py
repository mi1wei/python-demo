...
def goblin_meme_drission(metadata: dict):
    if len(metadata) < 10:
        return
    ...
    extension_url = f"https://www.goblin.meme/?referral_code=MWEYG4"
    ...
    try:
        ...
        # if page.ele('text=Connect your ', timeout=20):
        ...
        #     13: 3, 16: 1, 15: 3, 17: 3, 20: 1, 19: 1, 18: 4, 21: 2, 23: 1, 24: 3, 22: 4, 36: 4,
        ...
        #     71: 4, 75: 1, 73: 3, 74: 3, 76: 3, 78: 3, 77: 3, 79: 3, 80: 4, 83: 1, 82: 3,81:2,
        ...
        #         print(f"✅ 浏览器ID: {seq}, [{box['index']}] {box['name']}: Mining in progress… {page.ele('tag:p@class:text-3xl font-mono text-lime-400').text}")
        #         return
        #     else:
        #         if page.ele('text=Go to Mission 1', timeout=30):
        #             while page.ele('text=Go to Mission 1', timeout=5):
        #                 page.wait(3)
        #                 btn= page.ele('text=Go to Mission 1')
        #                 if btn:
        #                     btn.click()
        #         if page.ele('text=Go to Mission 2', timeout=3):
        #             while page.ele('text=Go to Mission 2', timeout=5):
        #                 page.wait(3)
        #                 btn = page.ele('text=Go to Mission 2')
        #                 if btn:
        #                     btn.click()
        #         if page.ele('text=Go to Mission 3', timeout=3):
        #             while page.ele('text=Go to Mission 3', timeout=5):
        #                 page.wait(3)
        #                 btn = page.ele('text=Go to Mission 3')
        #                 if btn:
        #                     btn.click()
        #         page.get(f'https://www.goblin.meme/box/{box["id"]}')
        ...
        #         if page.ele('text=Go to Mission 3', timeout=5):
        #             while page.ele('text=Go to Mission 3', timeout=5):
        #                 page.ele('text=Go to Mission 3').wait(2).click()
        #         if page.ele('text=OPEN BOX', timeout=5):
        #             page.ele('text=OPEN BOX').click()
        #             if page.ele('text=AWESOME!'):
        #                 page.ele('text=AWESOME!').click()
...
def bind_wallet(chromium, page, seq, extension_url="https://www.goblin.meme/wallet"):
    page.get(extension_url)
    if page.ele('text=Connected', timeout=10):
        print(f"✅ 浏览器ID: {seq}, Wallet Connected")
    elif page.ele('text=Connect MetaMask', timeout=20):
        page.ele('text=Connect MetaMask').click()
        page.wait(5)
        okx_tab = chromium.get_tab(url='mcohilncbfahbmgdjkbpemcciiolgcge')
        if okx_tab:
            handle_okx(okx_tab, seq)
        if page.ele('text=Switch Network', timeout=5):
            page.ele('text=Switch Network').click()
        print(f"✅ 浏览器ID: {seq}, Connect Wallet Succeed")
    else:
        error_browser_seq.append(seq)
    page.wait(10)
